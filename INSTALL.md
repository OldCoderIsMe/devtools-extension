# macOS 安装说明

## 安装步骤

### 方法一：从 DMG 安装（推荐）

1. 双击 `DevTools Suite-0.1.2.dmg` 文件
2. 将应用拖拽到 Applications 文件夹
3. 打开 Applications 文件夹，找到 "DevTools Suite"
4. 如果出现"无法打开，因为来自身份不明的开发者"的提示：
   - 右键点击应用
   - 选择"打开"
   - 在弹出的对话框中点击"打开"

### 方法二：从 ZIP 安装

1. 解压 `DevTools Suite-0.1.2-mac.zip` 文件
2. 将解压后的应用拖拽到 Applications 文件夹
3. 按照方法一中的步骤处理安全提示

## 如果无法安装（macOS 安全限制）

由于应用未进行代码签名，macOS 可能会阻止安装。解决方法：

### 方法 1：通过系统偏好设置允许

1. 打开"系统偏好设置" > "安全性与隐私"
2. 在"通用"标签页中，如果看到"已阻止使用 DevTools Suite，因为来自身份不明的开发者"
3. 点击"仍要打开"按钮

### 方法 2：使用命令行移除隔离属性

在终端中运行：

```bash
# 对于 DMG 文件
xattr -cr "/path/to/DevTools Suite-0.1.2.dmg"

# 对于已安装的应用
xattr -cr "/Applications/DevTools Suite.app"
```

### 方法 3：临时禁用 Gatekeeper（不推荐）

```bash
sudo spctl --master-disable
```

安装完成后，建议重新启用：

```bash
sudo spctl --master-enable
```

## 首次运行

1. 打开应用后，如果提示需要权限：
   - 点击"打开系统偏好设置"
   - 在"隐私与安全性"中授予相应权限

2. 应用启动后，可以使用以下快捷键：
   - `Cmd+K` 或 `Cmd+Space` - 打开快速搜索
   - 可以在设置中自定义快捷键

## 系统要求

### 最低系统版本
- **macOS 10.13 (High Sierra)** 或更高版本
- 原因：Electron 28 的最低系统要求

### 推荐系统版本
- **macOS 11.0 (Big Sur)** 或更高版本
- 原因：更好的兼容性和性能，完整功能支持

### 架构支持
- **Intel (x64)**：支持所有 Intel 芯片的 Mac
- **Apple Silicon (arm64)**：支持 M1/M2/M3 芯片的 Mac

### 功能限制说明

#### macOS 10.13-10.15
- ✅ 基本功能正常
- ⚠️ 部分视觉效果可能受限（如毛玻璃效果）
- ⚠️ 某些新特性可能不可用

#### macOS 11.0+
- ✅ 完整功能支持
- ✅ 最佳性能和体验
- ✅ 所有视觉效果正常

## 故障排除

### 应用无法启动

1. **检查 macOS 版本**
   - 最低要求：macOS 10.13 (High Sierra)
   - 推荐版本：macOS 11.0 (Big Sur) 或更高
   - 查看系统版本：Apple 菜单 > 关于本机

2. **检查应用权限设置**
   - 系统偏好设置 > 安全性与隐私 > 通用
   - 确保应用已被允许运行

3. **查看控制台日志**
   - 应用程序 > 实用工具 > 控制台
   - 搜索 "DevTools Suite" 查看错误信息

### 快捷键不工作

1. 打开应用设置（侧边栏 ⚙️ 图标）
2. 检查快捷键配置
3. 确保快捷键未被其他应用占用

### 其他问题

如果遇到其他问题，请：
1. 检查应用日志
2. 重新安装应用
3. 联系开发者
